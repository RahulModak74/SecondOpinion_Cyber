...
class EnhancedFAISSVectorStore:
    ...
    def create_embeddings(self, df, is_chain_data=False, batch_size=32, save_dir=None):
        """
        ...
        for i in range(0, len(self.texts), batch_size):
            ...
            print(f"Embedding batch {i//batch_size + 1}/{(len(self.texts)-1)//batch_size + 1}")
...
def process_incident_data(file_path, vector_store, chunksize=1000, temp_dir=None):
    """
    ...
    for chunk_num, chunk in enumerate(pd.read_csv(file_path, chunksize=chunksize)):
        ...
        vector_store.create_embeddings(chunk, is_chain_data=False, save_dir=temp_dir)
...
def process_chain_data(file_path, vector_store, temp_dir=None):
    """
    ...
    try:
        ...
        vector_store.create_embeddings(chains_df, is_chain_data=True, save_dir=temp_dir)
        ...
        process_incident_data(file_path, vector_store, chunksize=1000, temp_dir=temp_dir)
...
def main():
    ...
    incidents_file = "new_guide.csv"
    chains_file = "new_chain.csv"
    
    # Create temp directories
    temp_incidents_dir = "temp_incidents"
    temp_chains_dir = "temp_chains"
    
    # Step 1: Process incidents data
    print("\n=== Processing Security Incidents Data ===")
    ...
    try:
        ...
        process_incident_data(incidents_file, incidents_vector_store, 
                             chunksize=500, temp_dir=temp_incidents_dir)
    except Exception as e:
    ...
    try:
        ...
        process_chain_data(chains_file, chains_vector_store, temp_dir=temp_chains_dir)
    except Exception as e:
        print(f"Error with INSTRUCTOR model: {e}")
        print("Falling back to SentenceTransformer model")
        chains_vector_store = EnhancedFAISSVectorStore(
